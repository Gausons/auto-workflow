...
默认沿用来源设备和工作目录。系统读取固定边界内的用户消息、助手公开回复、工具记录及附件引用，并剔除 Codex 注入的插件列表、环境、技能和仓库指令等运行时包装；不迁移隐藏推理或工具授权。新会话自动归属原任务，继承记录可展开查看；再次切换时携带原上下文与新增对话，不重复嵌套之前的注入文本。

完整原始快照保存在 SQLite 的 `session_contexts` 表。首次发送时，系统从原始记录生成租户独立的 `.workflow-data/context/<tenant>/handoff-<id>-<version>.md`，按来源记录号列出用户目标、助手公开进展和历史摘录；长工具输出只在逐条证据文件中保留全文。配置了 `OPENAI_API_KEY` 时，默认使用 `AI_ASSIGNMENT_MODEL`（可由 `CONTEXT_SUMMARY_MODEL` 覆盖）对脱敏后的文本摘录再做模型归纳，每条结论附来源记录号；可设 `ENABLE_CONTEXT_SUMMARY=false` 关闭。模型不可用或整理失败会在 MD 中明确标注，并保留规则摘取。该步骤会把选取的历史文本发送到配置的 `OPENAI_BASE_URL`。超过内联预算时，新会话收到最近和开头的 Markdown 摘录及完整文档路径。历史中的 PNG、JPEG、GIF 和 WebP 图片会从 Base64 文本中提取到同目录的 `assets/`，按 SHA-256 去重，并以数据 URI 内嵌在完整 MD 的对应消息处，使该 MD 单文件包含原图。发给模型的轻量摘录使用相对图片路径；支持图片输入的 ACP Agent 还会收到原生图片内容块。
...
远端来源仍受连接器正文同步范围限制，并明确显示“部分记录”；目标须在同一设备的已注册工作目录运行。远端目标只接收可内联的 Markdown 摘录；历史图片原图和超长交接文档暂不能跨设备交付。跨设备代码/附件复制尚未实现。旧的手工交接记录与原会话续聊功能保留。
...
业务数据保存在 SQLite。数据库迁移会在启动时自动执行；第 4 版迁移保留缺陷与任务中心数据，并移除旧流水线运行和执行记录；第 5 版新增不可变会话上下文存储。
