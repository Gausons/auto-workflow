...
在历史会话底部选择目标 Agent，点击“新开会话”，即可进入继承上下文的新会话。输入框已有文字时会一起发送；没有文字时只创建工作台会话，首次发送时再创建原生 Agent 会话并注入上下文。无需整理摘要、填写任务或确认交接包。任务时间线也提供“打开会话 / 切换 Agent”入口。
...
完整上下文保存在 SQLite 的 `session_contexts` 表。超过内联字符预算时，系统保留开头和最近记录，将完整原文保存到租户独立的 `.workflow-data/context/` 文件供本机 Agent 按需读取。当前使用规则摘取，不调用模型生成摘要，也不保证所有图片可被目标 Agent 直接理解。
