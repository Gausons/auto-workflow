...
完整原始快照保存在 SQLite 的 `session_contexts` 表。超过内联字符预算时，系统保留开头和最近记录，并将一份使用附件引用的完整记录保存到租户独立的 `.workflow-data/context/` 文件供本机 Agent 按需读取。当前使用规则摘取，不调用模型生成摘要。历史中的 PNG、JPEG、GIF 和 WebP 图片会从 Base64 文本中提取到租户独立的 `.workflow-data/context/<tenant>/assets/`，按 SHA-256 去重并在发送前校验完整性；支持图片输入的 ACP Agent 会收到原生图片内容块，其他 Agent 收到包含原图文件路径、类型和校验值的降级引用。
...
远端来源仍受连接器正文同步范围限制，并明确显示“部分记录”；目标须在同一设备的已注册工作目录运行。跨设备代码/附件复制和远端长上下文文件交付尚未实现。旧的手工交接记录与原会话续聊功能保留。
