...
完整原始快照保存在 SQLite 的 `session_contexts` 表。首次发送时，系统从原始记录生成租户独立的 `.workflow-data/context/<tenant>/handoff-<id>-<version>.md`，按来源记录号列出用户目标、助手公开进展和全部可读取的历史记录（包括完整的长工具输出）。本机新会话收到该文件的路径和本轮消息，不再把历史正文重复塞入提示词；ACP 会话还会直接附带 MD 文件资源链接。配置了 `OPENAI_API_KEY` 时，默认使用 `AI_ASSIGNMENT_MODEL`（可由 `CONTEXT_SUMMARY_MODEL` 覆盖）对脱敏后的文本摘录再做模型归纳，每条结论附来源记录号；可设 `ENABLE_CONTEXT_SUMMARY=false` 关闭。模型不可用或整理失败会在 MD 中明确标注，并保留规则摘取。该步骤会把选取的历史文本发送到配置的 `OPENAI_BASE_URL`。历史中的 PNG、JPEG、GIF 和 WebP 图片会从 Base64 文本中提取到同目录的 `assets/`，按 SHA-256 去重，并以原始字节的数据 URI 内嵌在完整 MD 的对应消息处，不缩放或重新编码；支持图片输入的 ACP Agent 还会收到相同原始字节的原生图片内容块，Codex 原生通道也会收到对应的本地图片输入，以便视觉识别。单独存储的证据快照仅供内部核对，不要求 Agent 再读取第二个文件。
...
远端来源仍受连接器正文同步范围限制，并明确显示“部分记录”；目标须在同一设备的已注册工作目录运行。远端目标暂未实现单文件交付，仍只接收可内联的 Markdown 摘录；历史图片原图和超长交接文档暂不能跨设备交付。跨设备代码/附件复制尚未实现。旧的手工交接记录与原会话续聊功能保留。
