...
export function taskTimeline(task: any, data: any) {
  ...
  const items: any[] = sessions.map((s: any) => ({ kind: 'session', id: s.id, at: s.createdAt || s.updatedAt, value: s, jobs: jobs.filter((j: any) => j.conversationId ? j.conversationId === s.id : Boolean(s.nativeId) && s.deviceId === j.deviceId && s.agent === (j.agent || 'codex') && s.nativeId === (j.sessionId || j.threadId)) }));
