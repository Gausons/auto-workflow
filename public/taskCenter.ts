...
const executionLabels: any = { queued: '等待执行', launching: '正在创建会话', running: 'Codex 执行中', waiting: '等待你处理', completed: '本轮已完成', interrupted: '已停止', failed: '执行失败', unknown: '结果待核对' };
...
const selectFrom = (root: any, selector: string): any => selectFrom(root, selector);
...
export function createTaskCenterUI({ root, api, canEdit, toast }: any) {
  ...
  function executionCard(job: any) {
    ...
    return `<article class="tc-execution"><div class="tc-actions"><strong>${executionLabels[job.status]}</strong><span class="tc-meta">${time(job.createdAt)}</span></div><p>${esc(job.message)}</p>${job.desktopMessage ? `<p class="tc-meta">${esc(job.desktopMessage)}</p>` : ''}${job.controlError ? `<p role="alert">${esc(job.controlError)}</p>` : ''}${job.deviceId !== 'local' && job.threadId ? `<p class="tc-meta">目标设备的 Codex 会话：${esc(job.threadId)}</p>` : ''}<p class="tc-meta">${esc(job.cwd)} · 上下文 v${job.contextVersion}</p><div class="tc-actions">${link}${canEdit() && job.status === 'waiting' && job.request ? button('execution-respond', '处理 Codex 请求', job.id, true) : ''}${canEdit() && ['queued', 'running', 'waiting'].includes(job.status) ? button('execution-stop', job.status === 'queued' ? '取消等待' : '停止执行', job.id) : ''}${canEdit() && job.status === 'unknown' ? button('execution-reconcile', '核对执行结果', job.id) : ''}</div>${job.output ? `<details><summary>查看执行回复</summary><pre>${esc(job.output)}</pre></details>` : ''}</article>`;
  }
  function detail() {
  ...
  function editDialog(t: any = null, sessionId: any = null) {
    const s = data.sessions.find((s: any) => s.id === sessionId);
    ...
    onSubmit(d, async (f: any) => { const r = await mutate({ action: t ? 'update' : 'create', taskId: t?.id, revision: t?.revision, title: f.get('title'), status: f.get('status'), context: Object.fromEntries(Object.keys(fields).map(k => [k, f.get(k)])), sessionId }); selected = r.taskId; page = 'tasks'; });
  }
    ...
    try {
      ...
      if (action === 'execute-dialog') {
        ...
        try {
          ...
          const d = dialog('交给 Codex 执行', `<p>将在以下项目创建可在 Codex 客户端打开的新会话，并立即执行任务。</p><label>Codex 项目<select name="project">${result.projects.map((p: any, i: any) => `<option value="${i}">${esc(p.deviceName)} / ${esc(p.name)} · ${p.online ? '在线' : '离线，等待连接'} · ${esc(p.cwd)}</option>`).join('')}</select></label><h3>${esc(t.title)}</h3><p>${esc(t.context.next || t.context.goal || t.title)}</p><p class="tc-meta">使用目标 Codex 的登录和配置。执行过程中若需要确认，会在此任务中提示。</p>`, '立即执行');
          onSubmit(d, async (f: any) => { const p = result.projects[Number(f.get('project'))]; await api('/api/task-center/execute', { method: 'POST', body: JSON.stringify({ taskId: t.id, revision: t.revision, projectId: p.id, cwd: p.cwd, deviceId: p.deviceId }) }); tab = 'progress'; toast('已提交 Codex，执行状态将自动更新'); });
        } finally { b.disabled = false; }
      }
      ...
      if (action === 'link-dialog') {
        ...
        selectFrom(d, '#tc-from-session').onclick = () => { d.close(); editDialog(null, id); };
      ...
      if (action === 'session') {
        ...
        try { if (s.deviceId === 'local' && (s.source !== 'codexExecution' || s.historyId)) { const result = await api(`/api/agent-sessions/${s.historyId || s.id}?limit=100`); selectFrom(d, '#tc-messages').innerHTML = result.messages.map((m: any) => `<article class="tc-message"><strong>${esc(m.role)}</strong><pre>${esc(m.text || '非文本消息，请在 Agent 历史会话中查看')}</pre></article>`).join('') || '<p>暂无文本消息</p>'; }
