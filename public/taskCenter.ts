...
type UiElement = HTMLElement & { value: string; disabled: boolean; open: boolean; required: boolean; files: FileList | null };
type NamedForm = HTMLFormElement & { elements: HTMLFormControlsCollection & Record<string, UiElement> };
interface SessionCache { loading?: boolean; error?: string; messages?: HistoryMessage[]; total?: number; partial?: boolean }
...
interface TargetsResult { projects: AgentProject[]; localError?: string }
interface MutationResult { taskId: string; revision?: number; handoffId?: string }
interface SessionPage { messages: HistoryMessage[]; total: number; session?: { partial?: boolean } }
interface TaskCenterUiOptions { root: HTMLElement; api: Api; canEdit: () => boolean; toast: (message: string) => void }
...
const selectFrom = <T extends Element = UiElement>(root: ParentNode, selector: string): T => root.querySelector(selector) as T;
...
export function createTaskCenterUI({ root, api, canEdit, toast }: TaskCenterUiOptions) {
  ...
  let pendingData: TaskCenterData | null = null;
  let taskQuery = '';
  const signature = (value: TaskCenterData) => JSON.stringify({ ...value, devices: value.devices.map(({ lastSeen: _lastSeen, ...device }) => device) });
  const createdMessages: { text: string; taskId: string; executed: boolean }[] = [];
  const device = (id?: string) => data.devices.find(item => item.id === id);
  const position = (session: Session) => `${device(session.deviceId)?.name || '未知设备'} / ${session.agentLabel || session.agent || '未知 Agent'}`;
  const linked = (id: string) => data.tasks.find(item => item.sessionIds.includes(id));
  const task = () => data.tasks.find(item => item.id === selected);
  const requiredTask = (): Task => { const current = task(); if (!current) throw new Error('任务不存在'); return current; };
  const button = (action: string, title: string, id = '', primary = false) => `<button type="button" class="button ${primary ? 'primary' : 'secondary'}" data-tc="${action}" data-id="${esc(id)}">${title}</button>`;
  const empty = (title: string, body: string) => `<div class="tc-empty"><h2>${title}</h2><p>${body}</p></div>`;
  async function load({ quiet = false }: { quiet?: boolean } = {}) {
    ...
    try { const result = await api<TaskCenterData>('/api/task-center'); if (id !== request) return; if (quiet && loaded && signature(result) === signature(data)) return; if (quiet && loaded && page === 'tasks') { if (signature(result) !== signature(data)) { pendingData = result; const notice = selectFrom(root, '#tc-updates'); if (notice) notice.hidden = false; } return; } data = result; pendingData = null; loaded = true; if (!selected) { selected = data.tasks[0]?.id || null; const current = task(); const latest = current && taskTimeline(current, data).filter(i => i.kind === 'session').at(-1); if (latest) expanded.add(latest.id); } if (page !== 'new' || !selectFrom(root, '#tc-create-form')) render(); }
    catch (error) { if (id !== request) return; if (!quiet) { const message = errorMessage(error); if (!loaded) root.innerHTML = `<p role="alert">${esc(message)}</p>${button('refresh', '重试')}`; else toast(message); } }
  }
  function sessionCard(s: Session, assign = false) {
  ...
  function handoffCard(h: Handoff) {
  ...
  function executionCard(job: Execution) {
  ...
  function sessionBody(s: Session) {
    ...
    if (cached?.messages) return `<div class="history-messages">${renderMessages(cached.messages)}</div><p class="tc-meta">已显示 ${cached.messages.length} / ${cached.total || 0} 条记录${cached.partial ? ' · 部分记录' : ''}</p>${cached.messages.length < (cached.total || 0) ? button('more-session', '加载更多记录', s.id) : ''}`;
  ...
  async function readSession(id: string, more = false) {
    ...
    try {
      if (s.deviceId === 'local' && (!['codexExecution', 'agentExecution'].includes(s.source || '') || s.historyId)) {
        const result = await api<SessionPage>(`/api/agent-sessions/${encodeURIComponent(s.historyId || s.id)}?limit=100&offset=${more ? old?.messages?.length || 0 : 0}`);
    ...
    } catch (error) { sessionContent.set(id, { error: errorMessage(error) }); }
  ...
  function taskList() {
    ...
    return data.tasks.filter(t => !needle || [t.title, taskContent(t), ...data.sessions.filter(s => t.sessionIds.includes(s.id)).flatMap(s => [s.title, s.cwd, s.agent])].join(' ').toLowerCase().includes(needle))
      .sort((a, b) => Number(b.status === 'waiting') - Number(a.status === 'waiting') || taskActivity(b, data) - taskActivity(a, data))
      .map(t => `<button type="button" class="tc-task" data-tc="select" data-id="${esc(t.id)}" aria-pressed="${selected === t.id}"><strong>${esc(t.title)}</strong><span>${labels[t.status] || esc(t.status)}</span><small>${time(new Date(taskActivity(t, data)).toISOString())} · ${t.sessionIds.length} 段会话</small></button>`).join('') || '<p class="tc-meta">暂无匹配任务</p>';
  }
  function render() {
  ...
  function dialog(title: string, body: string, submit = '') {
  ...
  function onSubmit(d: HTMLDialogElement, handler: (form: FormData) => unknown | Promise<unknown>) {
    selectFrom<HTMLFormElement>(d, 'form').onsubmit = async e => { e.preventDefault(); const b = e.submitter as HTMLButtonElement | null; if (b) b.disabled = true;
      try { await handler(new FormData(e.currentTarget as HTMLFormElement)); d.close(); await load(); }
      catch (error) { selectFrom(d, '.tc-form-error').textContent = errorMessage(error); }
  ...
  async function loadBranches() {
    ...
    try { const result = await api<BranchState>('/api/task-center/git', { method: 'POST', body: JSON.stringify({ action: 'list', projectId: createProject()?.id, deviceId: createProject()?.deviceId, cwd: createCwd }) }); if (workspaceKey() === key) branchState = result; }
    catch (error) { if (workspaceKey() === key) branchError = errorMessage(error); }
  ...
  async function loadCreateTargets() {
    ...
    try {
      const result = await api<TargetsResult>('/api/task-center/codex');
    ...
    } catch (error) { createTargets = []; createTargetsError = errorMessage(error); }
    if (page === 'new') render();
  }
  async function openCreate(sessionId: string | null = null) {
    ...
    if (sessionId && !creating) {
      ...
      draft = data.sessions.find(s => s.id === sessionId)?.title || '';
      createError = '';
    }
    page = 'new'; render();
  ...
  root.addEventListener('submit', async e => {
    ...
    try {
      ...
      if (project) {
        ...
        catch (error) { createError = `任务已创建，但 Agent 启动失败：${errorMessage(error)}`; }
      }
      ...
      if (data.tasks.some(t => t.id === result.taskId)) { page = 'tasks'; if (createError) toast(createError); }
    } catch (error) { createError = errorMessage(error); }
    finally {
  ...
  root.addEventListener('keydown', e => {
    const event = e as KeyboardEvent, target = event.target as UiElement;
    if (event.key === 'Enter' && ['tc-branch-search', 'tc-new-branch', 'tc-create-cwd'].includes(target.id)) { event.preventDefault(); return; }
    if (target.id === 'tc-create-message' && event.key === 'Enter' && (event.metaKey || event.ctrlKey) && !event.isComposing) {
      event.preventDefault(); selectFrom<HTMLFormElement>(root, '#tc-create-form').requestSubmit();
    }
  });
  function editDialog(t?: Task) {
    if (!t) return;
  ...
  function handoffDialog() {
    ...
    function update() {
      ...
      const submit = form.querySelector('[type="submit"]'); if (submit) submit.textContent = mode === 'reference' ? '准备引用' : mode === 'branch' ? '创建分支与交接包' : '保存接续请求';
    }
  ...
  function packetText(h: Handoff) { const p = h.packet; return [`# ${p.title}`, `目标：${device(h.deviceId)?.name} / ${h.agent}`, `上下文版本：${p.contextVersion}`, h.targetSessionId ? `目标会话：${data.sessions.find(s => s.id === h.targetSessionId)?.nativeId || h.targetSessionId}` : '', taskContent(p), `## 下一位 Agent 的指令\n${p.instruction}`, '## 来源', ...p.sources.map(s => `${s.title} · ${s.agent} · ${s.nativeId}\n${s.cwd}\n${s.excerpt || '仅包含来源索引，可回到工作台查看原始记录。'}`), p.limitations].join('\n\n'); }
  root.addEventListener('input', e => {
    const target = e.target as UiElement;
    if (target.id === 'tc-branch-search') { const query = target.value.toLowerCase(); root.querySelectorAll<HTMLElement>('[data-tc="switch-branch"]').forEach(button => { button.hidden = !(button.dataset.id || '').toLowerCase().includes(query); }); return; }
    if (target.id === 'tc-task-query') { taskQuery = target.value; selectFrom(root, '#tc-task-results').innerHTML = taskList(); return; }
    if (target.id === 'tc-create-message') {
      draft = target.value;
  ...
  root.addEventListener('change', e => {
    const target = e.target as UiElement;
    if (target.id === 'tc-create-files') {
      const files = [...createFiles, ...Array.from(target.files || [])];
    ...
    if (target.id === 'tc-create-project') {
      createProjectIndex = Number(target.value) || 0; createModel = ''; createReasoningEffort = ''; createCwd = ''; render();
    }
    if (target.id === 'tc-create-model' || target.id === 'tc-create-effort') {
      if (target.id === 'tc-create-model') { createModel = target.value; createReasoningEffort = ''; }
      else createReasoningEffort = target.value;
      render();
      const menu = selectFrom(root, '#tc-create-model-menu'); if (menu) menu.open = true;
      selectFrom(root, `#${target.id}`)?.focus();
    }
  });
  root.addEventListener('keydown', e => {
    const event = e as KeyboardEvent, target = event.target as HTMLElement;
    if (event.key !== 'Escape') return;
    const menu = target.closest<HTMLDetailsElement>('.tc-config-menu');
    if (menu) { menu.open = false; menu.querySelector<HTMLElement>('summary')?.focus(); }
  });
  root.addEventListener('click', async e => {
    const target = e.target as HTMLElement;
    if (!target.closest('.tc-config-menu')) root.querySelectorAll<HTMLDetailsElement>('.tc-config-menu[open]').forEach(menu => { menu.open = false; });
    const b = target.closest<HTMLButtonElement>('[data-tc]'); if (!b) return; const id = b.dataset.id || '', action = b.dataset.tc || '';
    try {
      ...
      if (action === 'complete') { const t = requiredTask(); await mutate({ action: 'update', taskId: t.id, revision: t.revision, content: taskContent(t), status: t.status === 'completed' ? 'ready' : 'completed' }); await load(); return; }
      if (action === 'unlink-session' || action === 'move-session') {
        const t = requiredTask();
        const d = dialog(action === 'unlink-session' ? '解除会话关联' : '移动会话', action === 'unlink-session' ? '<p>保留原始会话，解除后可在未归属会话中找到。</p>' : `<label>目标任务<select name="targetTaskId" required>${data.tasks.filter(value => value.id !== t.id).map(value => `<option value="${esc(value.id)}">${esc(value.title)}</option>`).join('')}</select></label>`, '确认');
        onSubmit(d, f => mutate({ action: action === 'unlink-session' ? 'unlink' : 'move', taskId: t.id, revision: t.revision, sessionId: id, targetTaskId: f.get('targetTaskId'), targetRevision: data.tasks.find(value => value.id === f.get('targetTaskId'))?.revision })); return;
      }
      if (action === 'refresh') return await load();
      if (action === 'page') { page = id as Page; render(); }
      ...
      if (action === 'create-pick-directory') {
        ...
        b.disabled = true;
        try {
          const response = await api<DirectoryPickerResult>('/api/task-center/directory-picker', { method: 'POST', body: JSON.stringify({ deviceId: project.deviceId, projectId: project.id }) });
          if (response.status === 'completed') createCwd = response.cwd || '';
          else {
            for (let attempt = 0; attempt < 300; attempt++) {
              await new Promise(resolve => setTimeout(resolve, 1000));
              const status = await api<DirectoryPickerResult>(`/api/task-center/directory-picker?requestId=${encodeURIComponent(response.requestId)}`);
      ...
      if (action === 'execute-dialog') {
        ...
        try {
          ...
          const renderDirectories = () => {
            ...
            const choices = (items: string[], offset = 0) => `<div class="tc-actions">${items.map((cwd, index) => `<button type="button" class="button secondary" data-directory="${index + offset}">${esc(cwd)}</button>`).join('')}</div>`;
            ...
            d.querySelectorAll<HTMLButtonElement>('[data-directory]').forEach(button => { button.onclick = () => setDirectory(directories[Number(button.dataset.directory)]); });
          };
          const renderModels = () => {
            const p = result.projects[Number(form.elements.project.value)], previousModel = form.elements.model.value;
            form.elements.model.innerHTML = `<option value="">使用 Agent 默认模型${p.defaultModel ? `（${esc(p.defaultModel)}）` : ''}</option>${(p.models || []).map(model => `<option value="${esc(model.id)}">${esc(model.name || model.id)}</option>`).join('')}`;
            if ([...form.elements.model.querySelectorAll<HTMLOptionElement>('option')].some(option => option.value === previousModel)) form.elements.model.value = previousModel;
            const model = (p.models || []).find(item => item.id === form.elements.model.value), efforts = Array.isArray(model?.reasoningEfforts) ? model.reasoningEfforts : p.reasoningEfforts || [];
            form.elements.reasoningEffort.innerHTML = `<option value="">使用模型默认${model?.defaultReasoningEffort || p.defaultReasoningEffort ? `（${esc(model?.defaultReasoningEffort || p.defaultReasoningEffort)}）` : ''}</option>${efforts.map(effort => `<option value="${esc(effort.id)}">${esc(effort.name || effort.id)}</option>`).join('')}`;
          ...
          const pollDirectory = async (requestId: string) => {
            for (let attempt = 0; attempt < 300; attempt++) {
              await new Promise(resolve => setTimeout(resolve, 1000));
              const status = await api<DirectoryPickerResult>(`/api/task-center/directory-picker?requestId=${encodeURIComponent(requestId)}`);
          ...
          selectFrom(d, '#tc-pick-directory').onclick = async event => {
            const button = event.currentTarget as HTMLButtonElement, p = result.projects[Number(form.elements.project.value)]!; button.disabled = true;
            try {
              const response = await api<DirectoryPickerResult>('/api/task-center/directory-picker', { method: 'POST', body: JSON.stringify({ deviceId: p.deviceId, projectId: p.id }) });
      ...
      if (action === 'execution-respond') {
        const job = data.executions.find(value => value.id === id); if (!job?.request) throw new Error('交互请求不存在');
        const request = job.request, params = request.params || {};
        const questions = request.method === 'item/tool/requestUserInput' ? params.questions : null;
        const toolCall = params.toolCall && typeof params.toolCall === 'object' ? params.toolCall as { title?: string } : undefined;
        const body = questions ? questions.map((question, index) => `<label>${esc(question.question)}${question.options?.length ? `<p class="tc-meta">${question.options.map(option => esc(option.label + '：' + (option.description || ''))).join('<br>')}</p>` : ''}<textarea name="answer${index}" required maxlength="12000"></textarea></label>`).join('') : `<p>${esc(params.reason || toolCall?.title || 'Agent 请求执行以下操作')}</p><pre>${esc(params.command || JSON.stringify(params.toolCall || params, null, 2))}</pre><label>本次操作<select name="decision"><option value="decline">拒绝</option><option value="accept">允许本次操作</option></select></label>`;
        const d = dialog('回复 Agent', body, '发送回复');
        onSubmit(d, f => api('/api/task-center/execution-action', { method: 'POST', body: JSON.stringify({ executionId: id, action: 'respond', decision: f.get('decision'), ...(questions ? { answers: Object.fromEntries(questions.map((question, index) => [question.id, f.get('answer' + index)])) } : {}) }) }));
      }
      ...
      if (action === 'link-dialog') {
        ...
        const d = dialog('关联会话', `<label>选择已有任务<select name="taskId" required><option value="">请选择任务</option>${data.tasks.map(t => `<option value="${t.id}" ${t.id === selected ? 'selected' : ''}>${esc(t.title)}</option>`).join('')}</select></label><button class="button secondary" type="button" id="tc-from-session">用此会话创建任务</button>`, '关联任务');
        selectFrom(d, '#tc-from-session').onclick = () => { d.close(); openCreate(id); };
        onSubmit(d, async f => { const t = data.tasks.find(value => value.id === f.get('taskId')); if (!t) throw new Error('任务不存在'); await mutate({ action: 'link', taskId: t.id, revision: t.revision, sessionId: id }); selected = t.id; page = 'tasks'; });
      }
      if (action === 'session') {
        const s = data.sessions.find(value => value.id === id); if (!s) throw new Error('会话不存在');
        ...
        try { if (s.deviceId === 'local' && (!['codexExecution', 'agentExecution'].includes(s.source || '') || s.historyId)) { const result = await api<SessionPage>(`/api/agent-sessions/${s.historyId || s.id}?limit=100`); selectFrom(d, '#tc-messages').innerHTML = result.messages.map(message => `<article class="tc-message"><strong>${esc(message.role)}</strong><pre>${esc(message.text || '非文本消息，请在 Agent 历史会话中查看')}</pre></article>`).join('') || '<p>暂无文本消息</p>'; }
          else selectFrom(d, '#tc-messages').textContent = s.excerpt || '仅同步了会话索引，尚无文本片段。';
        } catch (error) { selectFrom(d, '#tc-messages').textContent = errorMessage(error); }
      }
      if (action === 'packet') {
        const h = data.handoffs.find(value => value.id === id); if (!h) throw new Error('交接记录不存在'); const content = packetText(h);
        ...
        selectFrom(d, '#tc-copy').onclick = async () => { try { await navigator.clipboard.writeText(content); toast('已复制'); } catch { selectFrom<HTMLTextAreaElement>(d, 'textarea').select(); selectFrom(d, '.tc-form-error').textContent = '请复制已选中的文本。'; } };
      ...
      if (action === 'started-dialog') {
        const h = data.handoffs.find(value => value.id === id); if (!h) throw new Error('交接记录不存在'); const candidates = data.sessions.filter(s => s.deviceId === h.deviceId && s.agent === h.agent && !linked(s.id));
    ...
    } catch (error) { toast(errorMessage(error)); b.disabled = false; }
  });
  root.addEventListener('toggle', e => { const target = e.target as HTMLDetailsElement; const id = target.dataset?.session; if (!id) return; if (target.open) { expanded.add(id); if (!sessionContent.has(id)) void readSession(id); } else expanded.delete(id); }, true);
