...
export class CodexRunner {
  ...
  async release(job) {
    ...
    try {
      const result = await client.call('thread/unsubscribe', { threadId: job.threadId });
      this.publish(job, { subscriptionStatus: result.status || 'unsubscribed', releasedAt: timestamp() });
    } catch (error) {
      this.publish(job, { releaseError: error.message, desktopMessage: '执行已经结束，但工作台未能释放会话。请重启工作台后再在 Codex 中继续。' });
    } finally {
