...
const required = (value: any, max = 12000) => { const s = text(value, max); if (!s) throw httpError(400, '请填写必填内容'); return s; };
const find = (items: any, id: any) => { const item = items.find((v: any) => v.id === id); if (!item) throw httpError(404, '记录不存在'); return item; };
const now = () => new Date().toISOString();
const sessionKey = (device: any, id: any) => createHash('sha256').update(`${device}\0${id}`).digest('hex');
const online = (device: any) => device.id === 'local' || Date.now() - Date.parse(device.lastSeen) < 90000;
const event = (task: any, message: any) => { task.events.unshift({ id: randomUUID(), at: now(), message }); task.updatedAt = now(); task.events = task.events.slice(0, 200); };

export function createTaskCenter({ database, tenantId, history }: any) {
  async function catalog() {
    const result = await history.catalog();
    return {
      device: { id: 'local', name: hostname(), agents: result.providers.map((p: any) => p.id), lastSeen: now(), transport: 'manual' },
      sessions: result.sessions.map((s: any) => ({ ...s, deviceId: 'local', nativeId: s.sessionId || s.id, historyId: s.id, excerpt: '' }))
    };
  }
  ...
  async function command(input: any, actor: any) {
    ...
    return database.mutateTaskCenter(tenantId, (data: any) => {
      ...
      const attach = (task: any, id: any) => {
        ...
        if (data.tasks.some((t: any) => t.id !== task.id && t.sessionIds.includes(id))) throw httpError(409, '会话已归属其他任务，请使用引用信息');
      ...
      switch (input.action) {
        ...
        case 'heartbeat': {
          ...
          let device = data.devices.find((d: any) => d.id === id);
        ...
        case 'handoff': {
          ...
          if (input.mode === 'continue' && data.handoffs.some((h: any) => h.taskId === task.id && h.mode === 'continue' && ['pending', 'received'].includes(h.status))) throw httpError(409, '已有待完成的接续，请先处理或取消');
          let targetSessionId: any = null;
