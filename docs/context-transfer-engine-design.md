...
当前落地：P0/P1 的包边界、数据源注册、Codex/Claude 会话解码适配、Markdown 来源、v1 快照统一校验；P2 的问题记录来源适配器、v3 图片对象拆分、HTTP 原始字节传输、第 7 版对象存储、可恢复的来源快照冻结，以及独立于任务的 v3 目录包导出、校验和快照文件导入。跨设备交接保留 v1/v2 读取兼容，目录包校验也保留 v2 支持。下文 P2/P3 中的业务宿主导入绑定、对象分片续传、普通附件、代码 patch、通用目标适配器及独立交付状态机仍是后续工作；当前执行归属和未知结果核对仍由工作台及现有执行器负责。
