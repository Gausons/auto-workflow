...
当前落地：P0/P1 的包边界、数据源注册、Codex/Claude 会话解码适配、Markdown 来源、v1 快照统一校验；跨设备路径使用 v2 清单包并保存第 6 版传输摘要记录，旧格式继续可读。目录包提供独立打包与校验命令。下文 P2/P3 中的对象分片、附件实际传送、代码 patch、通用目标适配器及独立交付状态机仍是后续工作；当前执行归属和未知结果核对仍由工作台及现有执行器负责。
