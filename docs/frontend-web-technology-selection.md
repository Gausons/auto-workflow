...
当前实施进度：Vite 内容哈希构建、manifest 静态资源加载、React/TanStack Query 基础入口、登录、组织初始化、账号密码修改、组织成员和审计记录、缺陷工作台、历史会话列表与详情、对接配置与分配规则已落地；历史续聊输入区和任务中心仍沿用兼容层，按下述阶段迁移。旧 hash 地址与服务端接口保持不变。
