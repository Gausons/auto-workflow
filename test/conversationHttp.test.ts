...
test('authenticated HTTP and real ACP subprocess support new conversation, two turns and switching back', async t => {
  ...
  const address: any = app.server.address(), base = `http://127.0.0.1:${address.port}`;
  async function req(route: string, method = 'GET', body?: any, token?: string) {
  ...
  const original = (await req('/api/agent-sessions', 'GET', undefined, token)).data.sessions[0].id;
  ...
  const id = created.data.sessionId, messageRoute = `/api/agent-sessions/${id}/continue`;
  ...
  const first = await send('按刚才的方案继续'); assert.match(first.output, /保持原接口兼容/);
  const second = await send('继续检查'); assert.match(second.output, /同一会话第 2 轮/); assert.equal(second.sessionId, first.sessionId);
