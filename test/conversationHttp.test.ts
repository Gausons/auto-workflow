...
test('authenticated HTTP and real ACP subprocess support new conversation, two turns and switching back', async t => {
  ...
  const probeBody = { executionId, deviceId: 'A', probe: detached.manifest };
  const transferRoute = `/api/conversations/${id}/transfer`;
  assert.equal((await req(transferRoute, 'POST', probeBody)).status, 401);
  assert.equal((await req(transferRoute, 'POST', probeBody, viewer)).status, 403);
  assert.equal((await req(transferRoute, 'POST', { ...probeBody, deviceId: 'B' }, token)).status, 403);
  assert.equal((await req(transferRoute, 'POST', { ...probeBody, probe: { schemaVersion: 3 } }, token)).status, 400);
  const initialProbe = await req(transferRoute, 'POST', probeBody, token);
  assert.equal(initialProbe.status, 200); assert.deepEqual(initialProbe.data.missingObjects, [object.digest]);
  ...
  const completedProbe = await req(transferRoute, 'POST', probeBody, token);
  assert.equal(completedProbe.status, 200); assert.deepEqual(completedProbe.data.missingObjects, []);
  const accepted = await req(transferRoute, 'POST', { executionId, deviceId: 'A', manifest: detached.manifest }, token);
